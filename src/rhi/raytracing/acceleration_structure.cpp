...
AccelerationStructure ASBuilder::Allocate(const D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS& inputs, uint64_t *scratchSize, const std::string& name)
{
    ...
    GPUResource* scratch = MakeBuffer(prebuildInfo.ScratchDataSizeInBytes, 0, D3D12_RESOURCE_STATE_COMMON, "Scratch Buffer");
    GPUResource* as = MakeBuffer(prebuildInfo.ResultDataMaxSizeInBytes, 0, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, name);
