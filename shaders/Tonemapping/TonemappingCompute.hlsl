...
void Main(uint3 ThreadID : SV_DispatchThreadID)
{
    ...
    if (ThreadID.x < Width && ThreadID.y < Height)
    {
        ...
        float3 MappedColor = pow(HDRColor, float4(1.0 / 2.2, 1.0 / 2.2, 1.0 / 2.2, 1.0 / 2.2)).xyz;
        //float3 MappedColor = clamp(HDRColor, float4(0.0, 0.0, 0.0, 0.0), float4(1.0, 1.0, 1.0, 1.0)).xyz;
